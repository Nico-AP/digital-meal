...
class TestTikTokAccessTokenService(TestCase):
    ...
    def test_update_or_create_creates_new_token(self):
        ...
        self.assertEqual(
            TikTokAccessToken.objects.filter(open_id="new_user_456").count(), 1
        )
    ...
    @patch("shared.portability.services.requests.post")
    @patch("shared.portability.services.time.sleep")
    def test_exchange_code_for_token_gives_up_after_max_attempts(
...
class TestTikTokPortabilityAPIClient(TestCase):
    ...
    def test_data_request_response_is_valid_when_ok_and_request_id_present(self):
        ...
        valid_response = {"error": {"code": "ok"}, "data": {"request_id": 12345}}

        is_valid, message = self.api_client.data_request_response_is_valid(
            valid_response
        )
    ...
    def test_data_request_response_is_valid_when_request_id_missing(self):
        ...
        invalid_response = {"error": {"code": "ok"}, "data": {}}
    ...
    def test_poll_status_response_is_valid_when_ok_and_valid_status(self):
        ...
        valid_response = {
            "error": {"code": "ok"},
            "data": {"request_id": 12345, "status": "pending"},
        }

        is_valid, message = self.api_client.poll_data_request_status_response_is_valid(
            valid_response
        )
    ...
    def test_poll_status_response_is_valid_when_status_code_invalid(self):
        ...
        invalid_response = {
            "error": {"code": "ok"},
            "data": {"request_id": 12345, "status": "unknown_status"},
        }
    ...
    def test_poll_status_response_is_valid_accepts_all_valid_statuses(self):
        ...
        for status in valid_statuses:
            response = {"error": {"code": "ok"}, "data": {"status": status}}

            is_valid, message = (
                self.api_client.poll_data_request_status_response_is_valid(response)
            )
    ...
    def test_cancel_data_request_success(self, mock_post):
        ...
        self.test_data_request.refresh_from_db()
        self.assertEqual(
            self.test_data_request.status, TikTokDataRequest.State.CANCELLED
        )
