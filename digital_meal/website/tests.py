...
@override_settings(
    DDM_SETTINGS={
        "EMAIL_PERMISSION_CHECK": r".*(\.|@)test\.com$",
    }
)
class DDMMiddlewareTest(TestCase):
    ...
    def test_staff_can_access_ddm(self):
        ...
        self.client.login(
            email="staff@test.com", username="staffuser", password="testpass123"
        )
        response = self.client.get(self.ddm_url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_superuser_can_access_ddm(self):
        ...
        self.client.login(
            email="admin@test.com", username="testadmin", password="testpass123"
        )
        response = self.client.get(self.ddm_url)
        self.assertEqual(response.status_code, 200)
